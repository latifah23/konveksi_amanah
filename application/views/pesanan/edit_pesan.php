<div class="content-wrapper">
	<div class="container">
		...
		<form role="form" action="<?= base_url('pesanan/action_update_pesanan') ?>" method="post" enctype="multipart/form-data">
								...
								<label >Kategori Produk</label>
									<select id="produk" class="form-control select jenisProduk" name="id_produk">
										...
										<?php foreach ($produk as $key => $value) : ?>
											<option value="<?= $value['id_produk']?>" <?php if ($pemesanan['id_produk']== $value['id_produk']){echo 'selected';}?>> <?= $value['nama_produk'] ?></option>
										<?php endforeach; ?>
									</select>
								</div>
							</div>
				...
				<div class="box box-primary">
					...
					<label for="input" class="col-sm-2 col-form-label">Jenis Kain</label>
						<div class="col-sm-10">
							<select id="kain" class="form-control select jenisKain" name="id_kain">
								...
								<option value="<?= $value['id_kain']?>"<?php if ($pemesanan['id_kain']== $value['id_kain']){echo 'selected';}?>> <?= $value['nama_kain'] ?></option>
								<?php endforeach; ?>
							</select>
						</div>
						<br>
					...
					<div class="col-sm-10">
							<select id="sablon" class="form-control select jenisKain" name="id_sablon">
								...
								<option value="<?= $value['id_sablon']?>"<?php if ($pemesanan['id_sablon']== $value['id_sablon']){echo 'selected';}?>> <?= $value['nama_sablon'] ?></option>
								<?php endforeach; ?>
							</select>
						</div>
						<br>
		...
		<form role="form" action="<?= base_url('pesanan/action_update_detail_pesanan') ?>" method="post" enctype="multipart/form-data">
			<table class="table table-bordered" id="customFields">
				...
				<tbody class="listDetail">
					<?php foreach ($detail_pesanan as $data):?>
						<input type="hidden" class="form-control" name="id_detail" value="<?= $data['id_detail'] ?>" hidden>
						<input type="hidden" class="form-control" name="id_pesan" value="<?= $data['id_pesan'] ?>" hidden>
							<tr>
								<td>
									<select class="form-control" name="ukuran">
										<option value="XS" <?php if ($data['ukuran'] == 'XS'){echo 'selected';}?>>XS</option>
										<option value="S"<?php if ($data['ukuran'] == 'S'){echo 'selected';}?>> S</option>
										...
										<option value="XXL"<?php if ($data['ukuran'] == 'XXL'){echo 'selected';}?>>XXL</option>
										<option value="XXXL"<?php if ($data['ukuran'] == 'XXXL'){echo 'selected';}?>>XXXL</option>
										<option value="Jumbo"<?php if ($data['ukuran'] == 'Jumbo'){echo 'selected';}?>>Jumbo</option>
									</select>
								</td>
								<td>
